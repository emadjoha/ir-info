...
class QgramIndex:
    ...
    def permutation(self, arr , terms , row , matcher):
        ...
        if (row == self.max_propTermArr):
            ...
            matcher.intersectionHandler.taked_terms = terms
            result = matcher.intersectionHandler.intersectMatchedDocs(matcher)

            if (result != -1):
                self.refreshCombPropDocs(result,matcher)
    ...
    def refreshCombPropDocs(self , new_result , matcher):
        # merging all proposal request  ....
        self.combinedPropDocs["sum_tf_idf"] = matcher.ranker.Union(self.combinedPropDocs["sum_tf_idf"], new_result["sum_tf_idf"])
        self.combinedPropDocs["vs_freq"] = matcher.ranker.Union(self.combinedPropDocs["vs_freq"], new_result["vs_freq"])
        self.combinedPropDocs["vs_tf_idf"] = matcher.ranker.Union(self.combinedPropDocs["vs_tf_idf"], new_result["vs_tf_idf"])
