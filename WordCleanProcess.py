...
class WordClean:
    ...
    def Tokenization(self , line):
        AbbrvWords , restOfLine = self.AmbToken.dealWithCommenAbbreviation(line)
        DTWords , restOfLine = self.AmbToken.dealWithDateTime(restOfLine)

        #print(DTWords)
        #print("rest " + restOfLine)
        words = re.split("[^a-zA-Z0-9]+", restOfLine)
        words.extend(DTWords)
        words.extend(AbbrvWords)
